...
@click.option('--train', required=False, is_flag=True, help='Runs on training data if supplied. Otherwise runs on test data')
...
def teach(patient, method, learning_algorithm, data, learnersaveto, plot_figures):
    ...
    if method.lower() == 'spectral':
        ...
        # click.secho('[Done]', fg='green')
        times, class_a_response, prediction, prediction_MA, prediction_KF = neural_power_core(sig=class_a_data_hstacked, fs=fs, bands=bands, band_names=band_names, online_window_size=online_window_size, fft_window_size=fft_win, fft_window_name='hann')
        print(f'{class_a_name} response dimensionality: {class_a_response.shape}')

        click.secho(f'Generating features for class: {class_b_name}')
        print(f'Input channels {class_b_name}: {class_b_data_hstacked.shape[0]}')
        print(f'Input length {class_b_name}: {class_b_data_hstacked.shape[1]}')
        times, class_b_response, prediction, prediction_MA, prediction_KF = neural_power_core(sig=class_b_data_hstacked, fs=fs, bands=bands, band_names=band_names, online_window_size=online_window_size, fft_window_size=2, fft_window_name='hann')
        print(f'{class_b_name} response dimensionality: {class_b_response.shape}')
        print(f'{method} times dimensionality: {times.shape}')
